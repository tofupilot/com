    ...
    <>
      <article className="px-6 pt-32 lg:px-8">
        <div className="mx-auto max-w-2xl text-base leading-7 text-zinc-700 dark:text-zinc-300">
              ...
              <li key={"categories"}>
                <div className="flex items-center gap-x-4">
                  <ChevronRightIcon
                    ...
                    aria-hidden="true"
                  />
                  {/* <BadgeTemplateCategory categories={post.categories} /> */}
                </div>
              </li>
            </ol>
          </nav>
          ...
          {/* Summary */}
          <p className="mt-6 text-xl leading-8">{template.summary}</p>
          ...
          <figure className="mt-10">
            <div className="relative z-0 mx-auto my-4 aspect-video max-w-screen-lg overflow-hidden rounded-lg shadow-xl shadow-black/40 dark:bg-zinc-950 dark:shadow-indigo-900/30">
              {template.mainVideo ? (
                <Iframe
                  url={getYoutubeNoCookieEmbedUrl(template.mainVideo)}
                  width="100%"
                  height="100%"
                  className={
                ...
                template.mainImage && (
                  <Image
                    ...
                    alt={template.mainImage?.alt || "Thumbnail"}
                    loading="eager"
                    fill
                    sizes="100vw"
          ...
          <div className="mt-6 flex items-center">
            <div className="flex-shrink-0">
              <a href={template.author.href}>
                <span className="sr-only">{template.author.name}</span>
                <Image
            ...
            <div className="ml-3">
              <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100">
                <a href={template.author.href}>{template.author.name}</a>
              </p>
              <div className="flex space-x-1 text-sm text-zinc-500">
                <time dateTime={template.publishedAt} className="font-normal">
                ...
                <span>{template.estReadingTime || "5"} min read</span>
              </div>
            </div>
          </div>
