import { urlForImage } from "@/app/(sanity)/lib/image";
import { Template } from "@/app/(sanity)/schemas/template";
import { Card } from "@/app/(home)/components/tremor/Card";
import Image from "next/image";
import Link from "next/link";

export default async function TemplateList({
  templates,
}: {
  templates: Template[];
}) {
  return (
    <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {templates.map((item) => (
        <Link href={"/templates/" + item.slug.current}>
          <Card
            key={item.title}
            className="group flex flex-col justify-between p-0 hover:cursor-pointer"
          >
            <div>
              <div className="aspect-h-1 aspect-w-1 lg:aspect-none h-44 w-full overflow-hidden rounded-t-md bg-zinc-200 object-cover group-hover:opacity-75">
                <Image
                  key={"tofupilot-logo"}
                  className="h-full w-full object-cover object-center lg:h-full lg:w-full"
                  src={urlForImage(item.mainImage)}
                  alt=""
                  height={200}
                  width={600}
                />
              </div>
              <div className="px-4 pb-2 pt-4">
                <dt className="truncate font-bold text-zinc-900 dark:text-white">
                  {item.title}
                </dt>
                <dd className="mt-2 text-zinc-700 dark:text-zinc-400">
                  {item.summary}
                </dd>
              </div>
            </div>
            <div className="mt-2 flex justify-between px-4 pb-4 pt-2">
              <div className="flex items-center gap-x-2 text-zinc-700 dark:text-zinc-400">
                <p>by</p>
                <div className="item-center flex -space-x-1 overflow-hidden">
                  <Image
                    key={"tofupilot-logo"}
                    className="inline-block size-5 rounded-full ring-1 ring-inset ring-zinc-900/7.5 group-hover:ring-zinc-900/10 dark:ring-white/10 dark:group-hover:ring-white/20"
                    src={urlForImage(item.author.image)}
                    alt=""
                    width={20}
                    height={20}
                  />
                </div>
                <p>{item.author.name}</p>
              </div>

              {/* Arrow */}
              <span
                aria-hidden="true"
                className="pointer-events-none absolute bottom-5 right-6 text-zinc-300 dark:text-zinc-400"
              >
                <svg
                  fill="currentColor"
                  viewBox="0 0 24 24"
                  className="h-4 w-4"
                >
                  <path d="M20 4h1a1 1 0 00-1-1v1zm-1 12a1 1 0 102 0h-2zM8 3a1 1 0 000 2V3zM3.293 19.293a1 1 0 101.414 1.414l-1.414-1.414zM19 4v12h2V4h-2zm1-1H8v2h12V3zm-.707.293l-16 16 1.414 1.414 16-16-1.414-1.414z" />
                </svg>
              </span>
            </div>
          </Card>
        </Link>
      ))}
    </dl>
  );
}
