...
export default function Infrastructure() {
  ...
  const features = [
    ...
    },
    {
      ...
      description:
        'Built for any volume of test data. From startups to global enterprises.',
    },
    {
      name: 'TofuPilot Graph Engine',
      description:
        'Designed with a powerful relational core for native correlation detection.',
    },
  ];
  ...
  return (
    <div className="px-3">
      <section
        ...
        className="relative mx-auto mt-28 flex w-full max-w-6xl flex-col items-center justify-center overflow-hidden rounded-3xl bg-zinc-950 pt-24 shadow-xl shadow-black/30 md:mt-40"
      >
            ...
            <div className="grid grid-cols-1 gap-x-10 gap-y-6 rounded-lg border border-white/[3%] bg-white/[1%] px-6 py-6 shadow-xl backdrop-blur md:grid-cols-3 md:p-8">
              {features.map(item => (
                <div key={item.name} className="flex flex-col gap-2">
                  ...
                  <p className="text-sm leading-6 text-lime-200/40">
                    {item.description}
                  </p>
                </div>
              ))}
